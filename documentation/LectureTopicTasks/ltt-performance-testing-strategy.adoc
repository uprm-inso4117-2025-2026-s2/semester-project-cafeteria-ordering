...
This LTT applies lecture concepts on performance classification, load modeling, metric separation, and performance regression prevention.
...
Performance Testing is treated as the overarching category. The following subtypes are included within this classification:

=== Baseline Performance Testing
Evaluation of individual system operations under normal load conditions to establish reference performance benchmarks.

=== Load Testing (Performance Subtype)
Evaluation of system behavior under expected peak cafeteria demand.

=== Stress Testing (Performance Subtype)
Evaluation of system behavior beyond expected load to identify failure thresholds.

=== Soak Testing (Performance Subtype)
...
* One-Time Password (OTP) locker validation service
...
* Parallel One-Time Password (OTP) validation requests.
