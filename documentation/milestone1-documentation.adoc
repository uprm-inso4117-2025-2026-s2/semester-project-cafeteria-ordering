...
*Notification system: The project will have different types of notifications, primarily:
** User notifications: the user will get notifications for the orders they submit; they get a notification when the order is being processed and when it is available to pick up.
** Staff notification: when a user makes an order, the cafeteria will receive a notification of the user’s order with details such as the dishes, snacks or drinks the user ordered.
