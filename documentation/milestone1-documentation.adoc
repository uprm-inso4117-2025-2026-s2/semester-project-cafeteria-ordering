...
The following selected fragments of the project implementation are crucial parts of the application that will be explained using source code, diagrams and pseudocode explinations. 
...
----
In this brief source code, createOrder takes an Order object as an input, said object has all the essential information such as student ID, selected items, price and status of the order. If there is an error such as the item not being in stock, an error will be displayed signaling that the order could not be completed, if it was completed, then the order will be placed!
...
| 
.Login Dark mode
 image:: designs/login_design/Login_dark_mode.png[Dark mode,180,300]
|====

The project counts on a light mode and a dark mode, for personal preference, as seen in the images above.
