...
| id
| UUID
| PRIMARY KEY, FK → auth.users.id (ON DELETE CASCADE)
| Profile ID (aligned with Supabase auth user ID)

| user_id
| UUID
| UNIQUE, FK → auth.users.id (ON DELETE CASCADE)
| Optional explicit reference to Supabase auth user
...
| User role: student, athlete, staff, or admin
...
* Role must be one of: 'student', 'athlete', 'staff', 'admin'
