...
* order class references Customer id inside database but this is not necessary when handling orders locally.
...
* To avoid conflicts between menu items in a single order, it is suggested to break the menu items and ingredients to an order_items table and link them to an order id in the orders table. With a one-to-many relationship between order and order_items.
