/*
TC-ORD-02: Place Order and Order Confirmation
...
export { dummyInvalidIDs, dummyOrderID, isValidUUID };

/*
Test Steps

. Call getOrderID() with a valid lowercase UUID and assert the return value matches.
. Call getOrderID() with undefined order_id and assert error 'Order must have a valid ID.' is thrown.
. Call getOrderID() with empty string order_id and assert error 'Order must have a valid ID.' is thrown.
. Call getOrderID() with each malformed UUID in dummyInvalidIDs and assert error 'Order ID must be a valid UUID.' is thrown.
